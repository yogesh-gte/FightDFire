...
<div class="dashboard-bar container-fluid container-xl d-flex justify-content-end">
	...
	<section class="hero-wrap js-fullheight" style="background-image: url('${pageContext.request.contextPath}/beauty/images/bg_1.jpg')" data-stellar-background-ratio="0.5">
					   ...
					   <section class="ftco-section user-split-section">
					     <div class="container-fluid">
					       ...
					       <div class="col-md-6 user-bg-left d-flex justify-content-center align-items-center p-4">
    ...
    <c:when test="${isFollowing}">
        <button type="button" class="btn btn-success mt-3 px-4" disabled>
            <i class="fas fa-check me-2"></i> Following
        </button>
    </c:when>
    <c:when test="${isPending}">
        <button type="button" class="btn btn-warning mt-3 px-4" disabled>
            <i class="fas fa-clock me-2"></i> Request Sent
        </button>
    </c:when>
    <c:otherwise>
        <form action="${pageContext.request.contextPath}/users/follow/${user.id}" method="post" style="display:inline;">
  ...
  <div class="container">
    ...
    <div class="row mb-4">
      <div class="col text-center">
        ...
        <p class="text-muted">${user.fullName}'s videos</p>
      </div>
    </div>
    ...
    <div class="row">
      <c:forEach var="video" items="${videos}">
        <div class="col-md-4 col-6 p-1">
          <div class="ratio ratio-1x1 bg-dark" style="border-radius: 4px; overflow: hidden; cursor: pointer;">
            <video class="w-100 h-100 object-fit-cover" onclick="this.paused ? this.play() : this.pause()">
              <source 
                src="${pageContext.request.contextPath}${video.videoPath}" 
                type="video/mp4">
            </video>
            <div class="position-absolute top-0 end-0 p-2">
              <c:if test="${not empty currentUser and currentUser.id == user.id}">
                <form method="post" action="${pageContext.request.contextPath}/video/deleteUpload"
                      onsubmit="return confirm('Delete this video?');" style="display:inline;">
                  <input type="hidden" name="videoId" value="${video.id}">
                  <button type="submit" class="btn btn-sm btn-danger p-1 opacity-75">
                      <i class="bi bi-trash-fill"></i>
                  </button>
                </form>
              </c:if>
            </div>
            <div class="position-absolute bottom-0 start-0 w-100 p-2 text-white bg-gradient-dark" style="background: linear-gradient(transparent, rgba(0,0,0,0.7));">
                <small><i class="bi bi-heart-fill"></i> ${video.likeCount} • <i class="bi bi-eye-fill"></i> ${video.viewCount}</small>
            </div>
          </div>
        </div>
      </c:forEach>
